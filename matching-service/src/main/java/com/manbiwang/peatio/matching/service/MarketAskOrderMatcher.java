 ...
 */
@Service
public class MarketAskOrderMatcher extends MarketOrderMatcher {
    ...
    ConcurrentNavigableMap<Integer, Order> getMarketOrders(OrderBook orderBook) {
        return orderBook.getMarketBidOrders();
    }
    ...
    Optional<Double> getBestLimitPrice(Trade trade) {
        ConcurrentNavigableMap<Double, ConcurrentNavigableMap<Integer, Order>> limitOrders =
                trade.getOrderBook().getLimitBidOrders();
        if (limitOrders.isEmpty()) {
            return Optional.empty();
        }
        if (limitOrders.firstEntry().getValue().isEmpty()) {
            return Optional.empty();
        }
        return Optional.of(limitOrders.firstEntry().getKey());
    }
}
